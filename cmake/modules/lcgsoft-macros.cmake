...
function(LCG_create_dependency_files)
  ...
  file(WRITE ${jsonfile} "{\n")
  file(WRITE ${sumfile} "# name-hash; package name; version; hash; short destination name; full directory name; comma separated dependencies\n")
  file(WRITE ${sumfile} "COMPILER: ${CMAKE_CXX_COMPILER_ID} ${CMAKE_CXX_COMPILER_VERSION}\n")
  ...
  foreach(name  ${LCG_externals} ${LCG_projects})
    if(NOT ${name}_exists)
      ...
      foreach(version ${${name}_native_version})
        ...
        set(sum_string "${name}-${${targetname}_hash}\;${name}\;${version}\;${${targetname}_hash}\;${${targetname}_dest_name}\;${${name}_directory_name}\;")
        set(firstdep 1)

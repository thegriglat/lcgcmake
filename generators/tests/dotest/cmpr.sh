...
print_tests_info() {
  ...
  # prepare environment
  if ! [ -s ./config.mk ] ; then
    echo "can't find ./config.mk  to include" >&2
    exit 1
  fi
  if ! [ -s ./libtests.mk ] ; then
    echo "can't find ./libtests.mk  to include" >&2
    exit 1
  fi
  
  # prepare string with values of all LIBS_*_PATH variables separated by ':' character
  local libpath=$(cat config.mk libtests.mk | grep "^LIBS_.*_PATH" | sed 's, ,,g' | cut -d = -f 2 | xargs | tr ' ' ':')
  export LD_LIBRARY_PATH=$libpath:$LD_LIBRARY_PATH
  
  echo "<h1>Test routines</h1>"
  
  local test_names=$(cat ${TEST_DAT} | cut -d ' ' -f 1 | sort -u)
  local svnweb="http://svnweb.cern.ch/world/wsvn/GENSER/validation/trunk/tests"
  
  for name in ${test_names} ; do
    echo "<h2 id=\"$name\" style=\"background: #EEE;\">${name}</h2>"
    ...
    echo "<p><b>Comments</b>:<br>"
    sed '/#@#/!d;s/^.*#@#//;s/$/<br>/;' ${name}.*
    echo "</p>"
    
    echo "<p><b>Dependencies:</b></p>"
    echo "<table border=\"1\">"
    echo "<tr> <th>Library</th> <th>Path</th> </tr>"
    ${LDD} bin/$name.exe | \
    sed -e 's,(.*$,,' -e 's,/afs/cern.ch/sw/lcg/external,EXTERNAL,' | \
    grep -vE "(linux-vdso|snoopy|libgcc_s|libstdc|libm|libc|libpthread|libdl|ld-linux)" | \
    while read line ; do
      local libname=${line%%=>*}
      local libpath=${line#*=>}
      echo "<tr> <td>${libname}</td> <td>${libpath}</td> </tr>"
    done
    echo "</table>"
  done
}

...
void runAlias() {
  ...
  dummy.readPDT("exampleFiles/evt.pdl");
...
void runRepeat(int nevent) {
  ...
  for(i=0;i<nevent;i++){
    ...
    EvtDecayTable* Mytable=EvtDecayTable::getInstance();
        
    Mytable->readDecayFile(std::string("DECAY.DEC"));
...
void runCPTest(int nevent, EvtGen &myGenerator) {
  ...
  //EvtGen myGenerator(decay_name,pdttable_name,myRandomEngine);
  myGenerator.readUDecay("./CPTEST.DEC");
...
void runDDalitz(int nevent, EvtGen &myGenerator, char* udec) {
    ...
    do{
      ...
      if(count == nevent-1) {
        ...
        testi << "evtgen_test1  1   " << val << "   " << errval << std::endl;
        val = m23->GetMean();
        errval = m23->GetMeanError();
        testi << "evtgen_test1  2   " << val << "   " << errval << std::endl;
      }
